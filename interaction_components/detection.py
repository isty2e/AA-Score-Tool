...
def filter_contacts(pairings):
    ...
    for contact in filtered1_pairings:
        try:
            dist = f"D{round(contact.distance, 2)}"
        except AttributeError:
            try:
                dist = f"D{round(contact.distance_ah, 2)}"
            except AttributeError:
                dist = f"D{round(contact.distance_aw, 2)}"
        res1, res2 = (
...
def hbonds(acceptors, donor_pairs, protisdon, typ):
    ...
    for acc, don in itertools.product(acceptors, donor_pairs):
        ...
        if not config.MIN_DIST < dist_ad < config.HBOND_DIST_MAX:
            continue
        vec1, vec2 = (
            vector(don.h_coords, don.d_coords),
            vector(don.h_coords, acc.coords),
        )
    ...
    for ring in rings:
        ...
        for p in pos_charged:
            ...
            if type(p).__name__ == "lcharge" and p.fgroup == "tertamine":
                ...
                # 'through' the ligand
                n_atoms = list(p.atoms[0].GetNeighbors())
...
def halogen(acceptor, donor):
    ...
    for acc, don in itertools.product(acceptor, donor):
        ...
        if not config.MIN_DIST < dist < config.HALOGEN_DIST_MAX:
            continue
        vec1, vec2 = (
            vector(acc.o_coords, acc.y_coords),
            vector(acc.o_coords, don.x_coords),
        )
        vec3, vec4 = (
            vector(don.x_coords, acc.o_coords),
            vector(don.x_coords, don.c_coords),
        )
...
def water_bridges(bs_hba, lig_hba, bs_hbd, lig_hbd, water):
    ...
    for w in water:
        ...
        for don1 in lig_hbd:
            ...
            d_angle = vecangle(
                vector(don1.h_coords, don1.d_coords),
                vector(don1.h_coords, w.oxy_coords),
            )
            if (
        ...
        for don2 in bs_hbd:
            ...
            d_angle = vecangle(
                vector(don2.h_coords, don2.d_coords),
                vector(don2.h_coords, w.oxy_coords),
            )
            if (
                config.WATER_BRIDGE_MINDIST <= dist <= config.WATER_BRIDGE_MAXDIST
                and d_angle > config.WATER_BRIDGE_THETA_MIN
    ...
    for l, p in itertools.product(lig_aw, prot_hw):
        ...
        w_angle = vecangle(
            vector(acc.coords, wl.oxy_coords), vector(wl.oxy_coords, don.h_coords)
        )
            ...
            for i, total in enumerate(all_total):
                ...
                if diff_to_next > 0.5:
                    ...
                    (
                        final_geom,
                        final_coo,
                    ) = next_total.geometry, next_total.coordination
