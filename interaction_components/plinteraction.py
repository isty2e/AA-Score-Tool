...
from interaction_components.detection import (
    halogen,
    hbonds,
    ...
    for atom in mol.GetAtoms():
        ...
        if symbol in ["O", "N", "S"]:
            if atomics.count(1) >= 1:
                ...
                symbol == "N"
                and atomics.count(1) == 0
                and atom.GetExplicitValence() <= 3
            ):
                acceptors.append(atom.GetIdx())
...
class Mol:
        ...
        for ring in rings_atom_idx:
            ...
            if 4 < len(ring) <= 6:
                ...
                if (
                    ...
                    or ring_is_planar(self.mol_conf, ring, r_atoms)
                ):
    ...
    def find_hal(self):
        ...
        for a in [at for at in self.mol.GetAtoms() if at.GetAtomicNum() in [8, 7, 16]]:
            n_atoms = [
                na for na in a.GetNeighbors() if na.GetAtomicNum() in [6, 7, 15, 16]
            ]
...
class Ligand(Mol):
    def __init__(self, mol, mol_water):
        ...
        self.max_dist_to_center = max(
            (
                euclidean3d(self.centroid, get_coord(self.mol_conf, a.GetIdx()))
        ...
        if mol_water is not None:
            ...
            for hoh in self.mol_water.GetAtoms():
                ...
                if hoh.GetAtomicNum() == 8:
                    oxy = hoh
                if oxy is not None and (
                    euclidean3d(self.centroid, get_atom_coords(oxy))
    ...
    def find_hba(self):
        ...
        return sorted(a_set, key=lambda x: x.a_orig_idx)

    def find_hbd(self):
        """Find all possible strong and weak hydrogen bonds donors (all hydrophobic C-H pairings)"""
        donor_pairs = []
        data = namedtuple(
            "hbonddonor", "d d_orig_atom d_orig_idx h type d_coords h_coords"
        )
        ...
        return sorted(donor_pairs, key=lambda x: (x.d_orig_idx, x.h.GetIdx()))

    def find_hal(self):
        """Look for halogen bond donors (X-C, with X=F, Cl, Br, I)"""
        data = namedtuple(
            "hal_donor", "x orig_x x_orig_idx c c_orig_idx x_coords c_coords"
        )
        a_set = []
    ...
    def find_charged(self):
        ...
        for a in self.mol.GetAtoms():
            ...
            if self.is_functional_group(a, "phosphate"):
                ...
                orig_contributing = list(a_contributing_orig_idx)
                a_set.append(
                    data(
            ...
            if self.is_functional_group(a, "sulfonicacid"):
                ...
                a_set.append(
                    data(
                        ...
                        atoms_orig_idx=a_contributing_orig_idx,
                        type="negative",
                        center=get_coord(self.mol_conf, a.GetIdx()),
                        fgroup="sulfate",
                    )
                )
            if self.is_functional_group(a, "carboxylate"):
                ...
                a_set.append(
                    data(
                        ...
                        center=centroid(
                            [
                                get_coord(self.mol_conf, a.GetIdx())
                                for a in a_contributing
                            ]
                        ),
                        fgroup="carboxylate",
                    )
    ...
    def is_functional_group(self, atom, group):
        ...
        if (
            group in ["quartamine", "tertamine"] and atom.GetAtomicNum() == 7
        ):  # Nitrogen
    ...
    def find_metal_binding(self):
        ...
        for a in self.mol.GetAtoms():
            ...
            if a.GetAtomicNum() == 8:  # Oxygen
                ...
                if (
                    True in [n.GetIsAromatic() for n in n_atoms]
            ...
            if a.GetAtomicNum() == 7 and n_atoms_atomicnum.count(6) == 2:
                a_set.append(
                    data(
                        atom=a,
                        atom_orig_idx=a_orig_idx,
...
class Protein(Mol):
    def __init__(self, mol):
        ...
        for a in [
            a for a in self.all_atoms if a.GetSymbol().upper() in config.METAL_IONS
        ]:
            m_orig_idx = a.GetIdx()
            orig_m = m_orig_idx
            self.metals.append(
    ...
    def hydrophobic_atoms(self):
        ...
        atm = [
            ...
            for a in self.all_atoms
            if a.GetAtomicNum() == 6
            and {natom.GetAtomicNum() for natom in a.GetNeighbors()}.issubset({1, 6})
        ]
        for atom in atm:
            orig_idx = atom.GetIdx()
            orig_atom = orig_idx
        ...
        for res in self.residues:
            ...
            if res.residue_name in ("ARG", "HIS", "LYS"):
                for a in res.residue_atoms:
                    if (
                        a.GetSymbol() == "N"
                        and a.GetPDBResidueInfo().GetName().strip(" ") != "N"
                    ):
            ...
            if res.residue_name in ("GLU", "ASP"):
                for a in res.residue_atoms:
                    if (
                        a.GetSymbol() == "O"
                        and a.GetPDBResidueInfo().GetName().strip(" ") != "O"
                    ):
                        a_contributing.append(a)
                        a_contributing_orig_idx.append(a.GetIdx())
                if a_contributing:
...
class PLInteraction:
    ...
    def __init__(self, lig_obj, bs_obj, pdbid):
        ...
        self.saltbridge_lneg = saltbridge(
            self.protein.get_pos_charged(), self.ligand.get_neg_charged(), True
        )
        self.saltbridge_pneg = saltbridge(
            self.ligand.get_pos_charged(), self.protein.get_neg_charged(), False
        )

        self.all_hbonds_ldon = hbonds(
            self.protein.get_hba(), self.ligand.get_hbd(), False, "strong"
        )
        self.all_hbonds_pdon = hbonds(
            self.ligand.get_hba(), self.protein.get_hbd(), True, "strong"
        )
        ...
        self.pication_paro = pication(
            self.protein.rings, self.ligand.get_pos_charged(), False
        )
        ...
        self.halogen_bonds = halogen(
            self.protein.halogenbond_acc, self.ligand.halogenbond_don
        )
        ...
        self.metal_complexes = metal_complexation(
            self.protein.metals, self.ligand.metal_binding, self.protein.metal_binding
        )
        ...
        self.unpaired_hba, self.unpaired_hbd, self.unpaired_hal = (
        ...
        self.interacting_chains = sorted(
            list({i.reschain for i in self.all_itypes if i.reschain not in [" ", None]})
        )
    ...
    def refine_hydrophobic(self, all_h, pistacks):
        ...
        for pistack, h in itertools.product(pistacks, all_h):
            ...
            brs, lrs = (
                [p1.GetIdx() for p1 in pistack.proteinring.atoms],
                [p2.GetIdx() for p2 in pistack.ligandring.atoms],
            )
            if h1 in brs and h2 in lrs:
                sel[(h1, h2)] = "EXCLUDE"
        hydroph = [
            h for h in all_h if (h.bsatom.GetIdx(), h.ligatom.GetIdx()) not in sel
        ]
        sel2 = {}
        ...
        for h in hydroph:
            if (h.ligatom.GetIdx(), h.resnr) not in sel2:
                sel2[(h.ligatom.GetIdx(), h.resnr)] = h
            elif sel2[(h.ligatom.GetIdx(), h.resnr)].distance > h.distance:
                sel2[(h.ligatom.GetIdx(), h.resnr)] = h
        hydroph = list(sel2.values())
        bsclust = {}
        ...
        idx_to_h = {}
        hydroph_final = [
            bsclust[bs][0] for bs in [a for a in bsclust if len(bsclust[a]) == 1]
        ]
        ...
        for bs in [a for a in bsclust if len(bsclust[a]) != 1]:
            ...
            for b in bsclust[bs]:
                ...
                neigh = list(b.ligatom.GetNeighbors())
                for n in neigh:
                    n_idx = n.GetIdx()
                    if n_idx in all_idx:
                        if n_idx < idx:
    ...
    def refine_hbonds_ldon(self, all_hbonds, salt_lneg, salt_pneg):
        ...
        # larger donor angle
        second_set = {}
        hbls = [k for k in i_set if not i_set[k]]
        for hbl in hbls:
            if hbl.d.GetIdx() not in second_set:
                second_set[hbl.d.GetIdx()] = (hbl.angle, hbl)
            elif second_set[hbl.d.GetIdx()][0] < hbl.angle:
                second_set[hbl.d.GetIdx()] = (hbl.angle, hbl)
    ...
    def refine_hbonds_pdon(self, all_hbonds, salt_lneg, salt_pneg):
        ...
        # larger donor angle
        second_set = {}
        hbps = [k for k in i_set if not i_set[k]]
        for hbp in hbps:
            if hbp.d.GetIdx() not in second_set:
                second_set[hbp.d.GetIdx()] = (hbp.angle, hbp)
            elif second_set[hbp.d.GetIdx()][0] < hbp.angle:
                second_set[hbp.d.GetIdx()] = (hbp.angle, hbp)
        return [hb[1] for hb in second_set.values()]
    ...
    def refine_water_bridges(self, wbridges, hbonds_ldon, hbonds_pdon):
        ...
        for wbridge in [
            ...
            if (wbridge.water.GetIdx(), wbridge.a.GetIdx()) not in wb_dict:
                ...
                omega - wb_dict[(wbridge.water.GetIdx(), wbridge.a.GetIdx())].w_angle
            ) < abs(omega - wbridge.w_angle):
                wb_dict[(wbridge.water.GetIdx(), wbridge.a.GetIdx())] = wbridge
        for wb_tuple, value in wb_dict.items():
            water, _ = wb_tuple
            if water not in wb_dict2:
                ...
                wb_dict2[water].append((abs(omega - value.w_angle), wb_dict[wb_tuple]))
                wb_dict2[water] = sorted(wb_dict2[water], key=lambda x: x[0])
            elif wb_dict2[water][1][0] < abs(omega - wb_dict[wb_tuple].w_angle):
                wb_dict2[water] = [
                    wb_dict2[water][0],
